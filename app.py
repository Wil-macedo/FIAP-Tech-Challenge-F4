...
from modelPredict import load_model as load_prediction_model, model_predict
import pandas as pd
import subprocess
import threading
import psutil
...
load_prediction_model()  # Garante que modelo está carregado, para rápida resposta.
        ...
        for key, value in data.items():
            if len(value) != 60:
                return make_response(
                    jsonify({"error": "Forneça exatamente 60 preços de fechamento para previsão."}),
                    400
                )
            ...
            log_df.to_csv(
                "log_predictions.csv",
                mode="a",
                header=not pd.io.common.file_exists("log_predictions.csv"),
                index=False
            )
